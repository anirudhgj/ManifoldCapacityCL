# Built upon https://raw.githubusercontent.com/huyvnphan/PyTorch_CIFAR10/master/cifar10_models/resnet.py
...
class ResNet(nn.Module):
    ...
    def embed(self, x):
        x = self.conv1(x) 
        x = self.norm1(x)
        x1 = self.activation(x)     # (N, 64, 64, 64)
        # x = self.maxpool(x)

        x2 = self.layer1(x1)        # (N, 64, 64, 64)
        x3 = self.layer2(x2)        # (N, 128, 32, 32)
        x4 = self.layer3(x3)        # (N, 256, 16, 16)
        x5 = self.layer4(x4)        # (N, 512, 8, 8)
